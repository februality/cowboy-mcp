                        ...
                        [
                            ...
                            'content' => [
                                ...
                                'text' => "Audit the WordPress site \"{$site_name}\" at {$site_url}.\n"
                                    ...
                                    . "- Use `wp_db_query` to count post revisions, transients, and spam comments.\n"
                        ...
                        [
                            ...
                            'content' => [
                                ...
                                'text' => "Guide me through importing content into \"{$site_name}\" from **{$format}** format.\n"
                                    ...
                                    . "- Recommend creating a backup before proceeding (use UpdraftPlus tools if available).\n"
                                    . "- Record current post count by type using `wp_db_query`:\n"
                                    . "  `SELECT post_type, post_status, COUNT(*) as count FROM wp_posts GROUP BY post_type, post_status`\n"
                                    . "\n"
                ...
                'render' => function ( array $args ) use ( $site_name ): array {
                    ...
                    $diagnostic_steps = match ( $symptom ) {
                        ...
                        'slow-loading' => "## Symptom-Specific Diagnosis: Slow Loading\n"
                            . "\n"
                            . "1. **Database health**: Run `wp_db_query` to check:\n"
                            . "   - `SELECT COUNT(*) FROM wp_posts WHERE post_type = 'revision'` -- excessive revisions\n"
                            . "   - `SELECT COUNT(*) FROM wp_options WHERE autoload = 'yes'` -- autoloaded options bloat\n"
                            . "   - `SELECT SUM(LENGTH(option_value)) FROM wp_options WHERE autoload = 'yes'` -- total autoload size\n"
                            ...
                            . "5. **Query analysis**: Use `wp_db_query` with `SHOW FULL PROCESSLIST` to check for slow queries.",
                        ...
                        'database-error' => "## Symptom-Specific Diagnosis: Database Error\n"
                            . "\n"
                            . "1. **Connection test**: Use `wp_db_query` with `SELECT 1` to verify database connectivity.\n"
                            . "2. **Table status**: Run `wp_get_tables` to check all tables are present.\n"
                            . "3. **Table repair**: Use `wp_db_query` with `CHECK TABLE` on suspected tables.\n"
                            . "4. **Corruption check**: Run `wp_db_write` with `REPAIR TABLE` on corrupted tables.\n"
                            . "5. **Options table**: Check autoload data size -- bloated `wp_options` is a common issue.",
                        ...
                        [
                            ...
                            'content' => [
                                ...
                                'text' => "Perform security hardening on \"{$site_name}\" at {$site_url}. Level: **{$level}**.\n"
                                    ...
                                    . "- [ ] Regular backup schedule configured (check UpdraftPlus if available)\n"
                                    . "\n"
                                    . "## Step 5: Report\n"
                        ...
                        [
                            ...
                            'content' => [
                                ...
                                'text' => "Optimize performance for \"{$site_name}\". Focus area: **{$area}**.\n"
                                    ...
                                    . "Run these diagnostic queries with `wp_db_query`:\n"
                                    . "- Post revisions: `SELECT COUNT(*) FROM wp_posts WHERE post_type = 'revision'`\n"
                                    . "- Auto-drafts: `SELECT COUNT(*) FROM wp_posts WHERE post_status = 'auto-draft'`\n"
                                    . "- Spam comments: `SELECT COUNT(*) FROM wp_comments WHERE comment_approved = 'spam'`\n"
                                    . "- Trashed comments: `SELECT COUNT(*) FROM wp_comments WHERE comment_approved = 'trash'`\n"
                                    . "- Transients: `SELECT COUNT(*) FROM wp_options WHERE option_name LIKE '_transient_%'`\n"
                                    . "- Autoloaded data size: `SELECT SUM(LENGTH(option_value)) as bytes FROM wp_options WHERE autoload = 'yes'`\n"
                                    . "- Orphaned postmeta: `SELECT COUNT(*) FROM wp_postmeta WHERE post_id NOT IN (SELECT ID FROM wp_posts)`\n"
                                    . "\n"
