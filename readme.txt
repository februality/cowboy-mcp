...
New AI apps can register with the site only for 15 minutes after an administrator opens Settings > Cowboy MCP > Connection (a countdown is shown there). Open that tab, then add the connector in your AI app. Connections that already exist are never affected by this, and reconnecting after a database sync does not need it either.
...
* Security: new AI apps can only register with the site for 15 minutes after an administrator opens the Connection tab or switches the connector on (a countdown shows there). Everything an existing connection uses - approval, tokens, refresh, the endpoint, recovery after a database sync - is unaffected, so nothing needs to be reconnected. The Connection Doctor reports the window; the cowboy_mcp_oauth_registration_open filter keeps it open for automated setups.
