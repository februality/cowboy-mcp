...
= 1.6.8 =
* Fix: OAuth connectors (Claude, ChatGPT) recover from a database overwrite. After a staging sync or backup restore replaced the site's database, the AI app kept a client registration the site no longer knew and landed on a dead-end "Unknown OAuth client" page; the only way out was removing and re-adding the connector. New connections now carry a signed registration that the site recognises without a database row, so recovery is Reconnect and Approve. A connection made before 1.6.8 can be rescued the same way: enable New connections on the Connection tab, click Reconnect in the app, and approve it - nothing has to be removed or re-added. Existing connections and tokens are untouched.
