...
* New: abilities registered by other plugins (WooCommerce, the AI plugin's core abilities, core) appear as tools in a new abilities category, with safe-mode confirmation and read-only scoping; they run their own permission checks and are not undoable.
* New: Connection Doctor reports the bridge state (registered, withheld, inbound counts; adapter presence).
* New: a one-time "How do you like Cowboy MCP so far?" prompt after the plugin has done real work on your site - a thumbs-up leads to the WordPress.org review form, a thumbs-down to a new support topic. Dismissible, snoozes for 30 days, never asks more than three times, and sends nothing anywhere.
